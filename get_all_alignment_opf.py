...
def get_pecha_ids_from_catalog(g):
    repo_path = download_repo("catlog", Path(f"./",))
    pecha_ids = []
...
if __name__ == "__main__":
    token = ""
    g = Github(token)
    pecha_ids = get_pecha_ids_from_catalog(g)
    Path(f"./ids/pecha_ids.txt").write_text(pecha_ids, encoding='utf-8')
    alignemnt_ids = get_all_alignment_ids(pecha_ids)
    Path(f"./ids/pecha_ids.txt").write_text(pecha_ids, encoding='utf-8')
